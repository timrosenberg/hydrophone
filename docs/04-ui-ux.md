...
- **Library**
  ...
  - **Home** is a distinct landing page, not just re-sorted album lists: a
    ...
    (re-roll button). Backed by `getAlbumList2` list types; shelves the
    server can't fill stay hidden; the played-based shelves are fed by the
    app's own scrobbling (see `02`).
- **Playlists**
